...
void ProjectOne::setup()
{
    ...
    Agent::add_model("Assets\\cube.sdkmesh", Agent::AgentModel::Cube);
    ...
    //mine
    auto villager = agents->create_behavior_agent("Villager", BehaviorTreeTypes::Villager);
    auto rabbit = agents->create_behavior_agent("Rabbit", BehaviorTreeTypes::Rabbit, Agent::AgentModel::Cube);
    auto fairy = agents->create_behavior_agent("Fairy", BehaviorTreeTypes::Fairy, Agent::AgentModel::Cube);
    auto wolf = agents->create_behavior_agent("Wolf", BehaviorTreeTypes::Wolf, Agent::AgentModel::Cube);
	...
	//villager
    villager->set_color(Vec3(0.3f, 0.5f, 1.0f));
    villager->set_position(Vec3(50.0f, 0.0f, 80.0f));
    villager->set_scaling(1.0f);

    //rabbit
    rabbit->set_color(Vec3(0.9f, 0.85f, 0.75f));
    rabbit->set_position(Vec3(100.0f, 0.0f, 100.0f));
    rabbit->set_scaling(0.25f);

    //fairy
    fairy->set_color(Vec3(1.0f, 0.4f, 0.9f)); //pink
    fairy->set_position(Vec3(80.0f, 0.0f, 80.0f));
    fairy->set_scaling(0.25f);

    //wolf
    wolf->set_color(Vec3(0.15f, 0.1f, 0.1f)); //black
    wolf->set_position(Vec3(60.0f, 0.0f, 60.0f));
    wolf->set_scaling(0.35f); 

    //hut
    //hut->set_color(Vec3(0.6f, 0.4f, 0.2f));
    //hut->set_position(Vec3(70.0f, 0.0f, 75.0f));
    //hut->set_scaling(1.f);
    ...
    for (int y = 0; y < 20; ++y)
        for (int x = 0; x < 20; ++x)
            terrain->pathLayer.set_value(x, y, (x < 10 ? Colors::DarkGreen : Colors::PaleGreen));
